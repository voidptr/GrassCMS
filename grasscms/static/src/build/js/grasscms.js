...
(function() {
  var PageEditor, PersistentGrass, Ruler, SimpleHtml5Editor, Sortable, assign_input_events_to_object, assign_toolbar_events, changed_input, contextual_menu, delete_, dragover, drop, getCurrentElement, getFromMenu, recover_mobility, setCss, setup_menu, setup_toolbar, stoppropagation, update_toolbar;
  ...
  assign_input_events_to_object = function(element) {
    var event, _i, _len, _ref, _results;
    _ref = ['input', 'change', 'blur'];
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      event = _ref[_i];
      _results.push(element.on(event, changed_input));
    }
    return _results;
  };
  ...
  assign_toolbar_events = function() {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      element = _ref[_i];
      _results.push(assign_input_events_to_object($(element)));
    }
    return _results;
  };

  changed_input = function(ev) {
    return ($(getCurrentElement().parent())).css(this.id, this.value);
  };
  ...
  update_toolbar = function(target) {
    ...
    return ($('#left')).val(((target.css('left')).split('px'))[0]);
  };
  ...
  setup_toolbar = function(toolbar) {
    ...
    return ($('#cssclasses')).on('change', function() {
      var class_, _i, _len, _ref;
      _ref = this.value.split(',');
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        class_ = _ref[_i];
        ($(getCurrentElement())).addClass(class_);
      }
      return ($(getCurrentElement())).trigger('changed', ['cssclasses', this.value]);
    });
  };
  ...
  dragover = function(ev) {
    ...
    return window.current_y = ev.pageY;
  };
  ...
  PersistentGrass = (function() {
    ...
    PersistentGrass.prototype.do_resizable = function() {
      ...
      return this.element = (($(this.element_)).wrap('<div draggable=true style="position:absolute; width:' + width + '; height:' + height + '" class="resizable" data-offset="' + this.options.offset + '">')).parent();
    };
    ...
    PersistentGrass.prototype.mouseout = function(ev) {
      ...
      if (!($(this)).id !== getCurrentElement().id) {
        return;
      }
